    ...
    public HttpResponse handleRequest(final HttpRequest req) throws TwitterException {
        ...
        for (int retriedCount = 0; retriedCount < retry; ++retriedCount) {
            ...
            try {
                ...
                HttpClientImpl.logger.debug("Sleeping " + this.CONF.getHttpRetryIntervalSeconds() + " seconds until the next retry.");
        ...
        if (this.isProxyConfigured()) {
            if (this.CONF.getHttpProxyUser() != null && !this.CONF.getHttpProxyUser().equals("")) {
                if (HttpClientImpl.logger.isDebugEnabled()) {
                    ...
                    HttpClientImpl.logger.debug("Proxy AuthPassword: " + this.CONF.getHttpProxyPassword().replaceAll(".", "*"));
                }
            ...
            final Proxy proxy = new Proxy(Proxy.Type.HTTP, InetSocketAddress.createUnresolved(this.CONF.getHttpProxyHost(), this.CONF.getHttpProxyPort()));
            if (HttpClientImpl.logger.isDebugEnabled()) {
                HttpClientImpl.logger.debug("Opening proxied connection(" + this.CONF.getHttpProxyHost() + ":" + this.CONF.getHttpProxyPort() + ")");
            }
