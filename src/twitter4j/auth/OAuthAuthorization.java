...
public class OAuthAuthorization implements Authorization, Serializable, OAuthSupport {
	...
	public String getAuthorizationHeader(final HttpRequest req) {
		return this.generateAuthorizationHeader(req.getMethod().name(), req.getURL(), req.getParameters(),
				this.oauthToken);
	}
	...
	public RequestToken getOAuthRequestToken(final String callbackURL, final String xAuthAccessType,
		...
		this.oauthToken = new RequestToken(OAuthAuthorization.http.post(this.conf.getOAuthRequestTokenURL(),
	...
	public AccessToken getOAuthAccessToken() throws TwitterException {
		...
		this.oauthToken = new AccessToken(
				OAuthAuthorization.http.post(this.conf.getOAuthAccessTokenURL(), null, this, null));
		return (AccessToken) this.oauthToken;
	}
	...
	public AccessToken getOAuthAccessToken(final String oauthVerifier) throws TwitterException {
		this.ensureTokenIsAvailable();
		this.oauthToken = new AccessToken(OAuthAuthorization.http.post(this.conf.getOAuthAccessTokenURL(),
	...
	public AccessToken getOAuthAccessToken(final String screenName, final String password) throws TwitterException {
		try {
			...
			this.oauthToken = new AccessToken(OAuthAuthorization.http.post(url,
					new HttpParameter[] { new HttpParameter("x_auth_username", screenName),
	...
	String generateAuthorizationHeader(final String method, final String url, HttpParameter[] params,
		...
		final StringBuilder base = new StringBuilder(method).append("&")
	...
	String generateAuthorizationHeader(final String method, final String url, final HttpParameter[] params,
			final OAuthToken token) {
		final long timestamp = System.currentTimeMillis() / 1000L;
		final long nonce = timestamp + OAuthAuthorization.RAND.nextInt();
		return this.generateAuthorizationHeader(method, url, params, String.valueOf(nonce), String.valueOf(timestamp),
				token);
	}
	...
	public String toString() {
		return "OAuthAuthorization{consumerKey='" + this.consumerKey + '\''
				+ ", consumerSecret='******************************************'" + ", oauthToken=" + this.oauthToken
				+ '}';
	}
}
