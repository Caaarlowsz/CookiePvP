...
public class OAuth2Authorization implements Authorization, Serializable, OAuth2Support {
	...
	public String getAuthorizationHeader(final HttpRequest req) {
		if (this.token == null) {
			String credentials = "";
			try {
				credentials = String.valueOf(URLEncoder.encode(this.consumerKey, "UTF-8")) + ":"
						+ URLEncoder.encode(this.consumerSecret, "UTF-8");
	...
	public String toString() {
		return "OAuth2Authorization{consumerKey='" + this.consumerKey + '\''
				+ ", consumerSecret='******************************************'" + ", token="
				+ ((this.token == null) ? "null" : this.token.toString()) + '}';
	}
}
