...
public class APIStatisticsOpenMBean implements DynamicMBean
{
    ...
    public APIStatisticsOpenMBean(final APIStatistics apiStatistics) {
        ...
        try {
            this.METHOD_STATS_TYPE = new CompositeType("method statistics", "method statistics", APIStatisticsOpenMBean.ITEM_NAMES, APIStatisticsOpenMBean.ITEM_DESCRIPTIONS, APIStatisticsOpenMBean.ITEM_TYPES);
    ...
    public MBeanInfo getMBeanInfo() {
        ...
        constructors[0] = new OpenMBeanConstructorInfoSupport("APIStatisticsOpenMBean", "Constructs an APIStatisticsOpenMBean instance", (OpenMBeanParameterInfo[])new OpenMBeanParameterInfoSupport[0]);
        ...
        final OpenMBeanInfoSupport info = new OpenMBeanInfoSupport(this.getClass().getName(), "API Statistics Open MBean", attributes, constructors, (OpenMBeanOperationInfo[])operations, notifications);
        return info;
    }
    ...
    public synchronized TabularDataSupport getStatistics() {
        ...
        for (final InvocationStatistics methodStats : this.API_STATISTICS.getInvocationStatistics()) {
            final Object[] itemValues = { methodStats.getName(), methodStats.getCallCount(), methodStats.getErrorCount(), methodStats.getTotalTime(), methodStats.getAverageTime() };
            try {
                final CompositeData result = new CompositeDataSupport(this.METHOD_STATS_TYPE, APIStatisticsOpenMBean.ITEM_NAMES, itemValues);
