...
public class TellCommand implements CommandExecutor, Listener
{
    ...
    public void flodar(final PlayerCommandPreprocessEvent e) {
        ...
        if (TellCommand.cooldown.contains(p) && TellCommand.cooldown.contains(p) && e.getMessage().toLowerCase().startsWith("/tell")) {
            e.setCancelled(true);
        ...
        if (Cmd.getName().equalsIgnoreCase("tell")) {
            ...
            if (Args[0].equalsIgnoreCase("off")) {
                if (cfConfig.pegarconfig().get("configura\u00e7\u00e3o." + p.getUniqueId() + ".Tell").equals("Ativado")) {
                    ...
                    cfConfig.pegarconfig().set("configura\u00e7\u00e3o." + p.getUniqueId() + ".Tell", (Object)"Desativado");
                    cfConfig.salvarconfiguracao();
                }
                else {
