...
public class TpCommand implements CommandExecutor {
	...
	public boolean onCommand(final CommandSender Sender, final Command Cmd, final String Label, final String[] Args) {
		...
		if (Cmd.getName().equalsIgnoreCase("tp")) {
			...
			if (Args.length == 0) {
				p.sendMessage("§6§lTELEPORT §fUtilize §6§l/tp §f(jogador) (alvo) §6| §6§l[X] [Y] [Z]");
				return true;
			}
			if (Args.length > 2 || Args.length == 3) {
				if (!this.CheckarNumero(Args[0])) {
					p.sendMessage(
							"§c\u00c9 necess\u00e1rio utilizar apenas n\u00fameros para teleportar para uma §c§lcoordenada.");
					return true;
				}
				...
				p.sendMessage(
						"§7Voc\u00ea foi §6§lTELEPORTADO §7at\u00e9 as coordenadas: §6§l" + X + " " + Y + " " + Z);
