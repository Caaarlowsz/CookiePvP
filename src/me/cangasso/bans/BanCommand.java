	...
	public boolean onCommand(final CommandSender sender, final Command cmd, final String label, final String[] args) {
		...
		if (vitima == null) {
			...
			if (Config.getConfig().getBans().get("Bans." + vitimaoff.getUniqueId()) != null) {
				API.sendMsg((CommandSender) jogador,
						"§fO jogador §c§l" + vitimaoff.getName() + " §fj\u00e1 est\u00e1 §4§lBANIDO§f!");
				API.sendSound(jogador, Sound.NOTE_PLING, 10);
				return true;
			}
			if (Config.getConfig().getIpBans().get("IPBans." + vitimaoff.getUniqueId()) != null) {
				API.sendMsg((CommandSender) jogador,
						"§fO jogador §c§l" + vitimaoff.getName() + " §fj\u00e1 est\u00e1 §4§lBANIDO§4(IP)§f!");
				API.sendSound(jogador, Sound.NOTE_PLING, 10);
				return true;
			}
			if (Config.getConfig().getTempBans().get("TempBans." + vitimaoff.getUniqueId()) != null) {
				API.sendMsg((CommandSender) jogador,
						"§fO jogador §c§l" + vitimaoff.getName() + " §fj\u00e1 est\u00e1 §9§lBANIDO§9(tempban)§f!");
				API.sendSound(jogador, Sound.NOTE_PLING, 10);
				return true;
			}
			...
			API.sendStaff(
					"§c" + jogador.getName() + " §cbaniu o jogador §c" + vitimaoff.getName() + "§c. Motivo: " + Motivo);
			...
			if (Config.getConfig().getBans().get("Bans." + vitima.getUniqueId()) != null) {
				API.sendMsg((CommandSender) jogador,
						"§cO jogador " + vitima.getName() + " §cj\u00e1 est\u00e1 banido!");
				API.sendSound(jogador, Sound.NOTE_PLING, 10);
				return true;
			}
			...
			API.sendStaff(
					"§c" + jogador.getName() + " §cbaniu o jogador §c" + vitima.getName() + "§c. Motivo: " + Motivo2);
