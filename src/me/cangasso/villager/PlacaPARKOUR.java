...
public class PlacaPARKOUR implements Listener
{
    ...
    public void clicar(final PlayerInteractEvent e) {
        ...
        if (e.getAction() == Action.RIGHT_CLICK_BLOCK && e.getClickedBlock() != null && (e.getClickedBlock().getType() == Material.WALL_SIGN || e.getClickedBlock().getType() == Material.SIGN_POST) && API.getWarp(p) == "Parkour") {
            final Sign s = (Sign)e.getClickedBlock().getState();
            final String[] lines = s.getLines();
            if (lines.length > 0 && lines[0].equals(" ") && lines.length > 1 && lines[1].equals("§6§lPARKOUR") && lines.length > 2 && lines[2].equals(" §7(Clique)") && lines.length > 3 && lines[3].equals(" ")) {
