    ...
    public void onPlayerQuitEvent(final PlayerQuitEvent Evento) {
        ...
        if (Main.Evento == Estado.INICIANDO && PlayerAPI.Participando.contains(Jogador)) {
            ...
            if (!PlayerAPI.Batalhando.contains(Jogador) && PlayerAPI.Participando.contains(Jogador)) {
                PlayerAPI.RemoverRDM(Jogador);
                PlayerAPI.Participando.remove(Jogador);
                Tempo.mandarBroadcast("§7O jogador §6§l" + Jogador.getName() + "§7 saiu do evento §e§l(" + PlayerAPI.Participando.size() + "§e§l/30)");
                return;
            }
            if (PlayerAPI.Batalhando.contains(Jogador) && PlayerAPI.Participando.contains(Jogador) && PlayerAPI.Batalhando1.get(Jogador) == Jogador.getName() && PlayerAPI.Ganhador.get(Jogador) == Jogador.getName()) {
                final Player Ganhador = Bukkit.getPlayer((String)PlayerAPI.Batalhando2.get(Jogador));
                PlayerAPI.Batalhando.remove(Jogador);
                PlayerAPI.Participando.remove(Jogador);
                Tempo.mandarBroadcast("§7O jogador §c§l" + Jogador.getName() + " §7n\u00e3o aguentou e desistiu do duelo contra §a§l" + Ganhador.getName() + "§7. §e§l(" + PlayerAPI.Participando.size() + "§e§l/30)");
                Tempo.mandarBroadcast("§7O jogador §a§l" + Ganhador.getName() + " §7venceu o duelo contra §c§l" + Jogador.getName() + "§7. §e§l(" + PlayerAPI.Participando.size() + "§e§l/30)");
            ...
            if (PlayerAPI.Batalhando.contains(Jogador) && PlayerAPI.Participando.contains(Jogador) && PlayerAPI.Batalhando1.get(Jogador) == Jogador.getName() && PlayerAPI.Ganhador.get(Jogador) != Jogador.getName()) {
                ...
                PlayerAPI.Batalhando.remove(Jogador);
                PlayerAPI.Participando.remove(Jogador);
                Tempo.mandarBroadcast("§7O jogador §c§l" + Jogador.getName() + " §7n\u00e3o aguentou e desistiu do duelo contra §a§l" + Ganhador.getName() + "§7. §6(" + PlayerAPI.Participando.size() + "/30)");
                Tempo.mandarBroadcast("§7O jogador §a§l" + Ganhador.getName() + " §7venceu o duelo contra §c§l" + Jogador.getName() + "§7. §e§l(" + PlayerAPI.Participando.size() + "§e§l/30)");
            ...
            if (PlayerAPI.Batalhando.contains(Jogador) && PlayerAPI.Participando.contains(Jogador) && PlayerAPI.Batalhando2.get(Jogador) == Jogador.getName() && PlayerAPI.Ganhador.get(Jogador) == Jogador.getName()) {
            ...
            if (PlayerAPI.Batalhando.contains(Jogador) && PlayerAPI.Participando.contains(Jogador) && PlayerAPI.Batalhando2.get(Jogador) == Jogador.getName() && PlayerAPI.Ganhador.get(Jogador) != Jogador.getName()) {
                ...
                PlayerAPI.Participando.remove(Jogador);
                Tempo.mandarBroadcast("§7O jogador §c§l" + Jogador.getName() + " §7n\u00e3o aguentou e desistiu do duelo contra §a§l" + Ganhador.getName() + "§7. §6(" + PlayerAPI.Participando.size() + "/30)");
                Tempo.mandarBroadcast("§7O jogador §a§l" + Ganhador.getName() + " §7venceu o duelo contra §c§l" + Jogador.getName() + "§7. §6(" + PlayerAPI.Participando.size() + "/30)");
    ...
    public void onPlayerDeathEvent(final PlayerDeathEvent Evento) {
        ...
        if (Matador != Vitima) {
            if (Main.Evento == Estado.ANDAMENTO && PlayerAPI.Participando.contains(Matador) && PlayerAPI.Participando.contains(Vitima)) {
                ...
                Tempo.mandarBroadcast("§7O jogador §a§l" + Matador.getName() + " §7venceu o duelo contra §c§l" + Vitima.getName() + "§7. §e§l(" + PlayerAPI.Participando.size() + "§e§l/30)");
                if (PlayerAPI.Ganhador.get(Matador) == Matador.getName()) {
                    ...
                    Tempo.mandarBroadcast("§a§lGANHADOR: §fO jogador §2§l" + Matador.getName() + " §7\u00e9 o vencedor do evento!");
                }
        ...
        if (PlayerAPI.Participando.contains(Jogador) && !PlayerAPI.Batalhando.contains(Jogador)) {
            if (!Evento.getMessage().startsWith("/sair") && !Evento.getMessage().startsWith("/rdm")) {
                if (!Jogador.hasPermission("RDM.Admin")) {
                    ...
                    Jogador.sendMessage("§cVoc\u00ea s\u00f3 pode utilizar os comandos §c§l/sair §ce §c§l/rdm §cneste local.");
                }
                Evento.setCancelled(true);
            }
        }
