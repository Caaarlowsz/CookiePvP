        ...
        if (Comando.getName().equalsIgnoreCase("RDM") || Comando.getName().equalsIgnoreCase("ReiDaMesa")) {
            ...
            if (Args[0].equalsIgnoreCase("Iniciar") || Args[0].equalsIgnoreCase("Start")) {
                if (Main.Evento == Estado.INICIANDO) {
                    Jogador.sendMessage("§6§lCookie§f§lNetwork §7» §fN\u00e3o foi poss\u00edvel iniciar este §5§lEVENTO §fpois j\u00e1 h\u00e1 outro sendo iniciado!");
                    return true;
                }
                if (Main.Evento == Estado.ANDAMENTO) {
                    Jogador.sendMessage("§6§lCookie§f§lNetwork §7» §fN\u00e3o foi poss\u00edvel iniciar este §5§lEVENTO §fpois j\u00e1 h\u00e1 outro em andamento!");
                    return true;
                }
                ...
                if (Main.Evento == Estado.ANDAMENTO) {
                    Jogador.sendMessage("§cN\u00e3o foi poss\u00edvel cancelar o evento pois j\u00e1 h\u00e1 outro em andamento!");
                    return true;
                }
                ...
                if (Main.Evento == Estado.ANDAMENTO) {
                    Jogador.sendMessage("§cVoc\u00ea n\u00e3o pode alterar o tempo pois o evento j\u00e1 est\u00e1 em andamento!");
                    return true;
                }
                if (Args.length == 1) {
                ...
                if (Time == 0 || Time > 301) {
                    Jogador.sendMessage("§cVoc\u00ea s\u00f3 pode utilizar n\u00fameros acima de §c§l60 segundos §cou §c§l301 segundos §8(5 minutos)");
                    return true;
                }
                Jogador.sendMessage("§7Voc\u00ea alterou o tempo para: §a§l" + Time + " §7segundo(s)");
                Tempo.mandarBroadcast("§6§lCookie§f§lNetwork §7» O tempo foi alterado para §a§l" + Time + " §7segundo(s)");
            ...
            if (Main.Evento == Estado.FECHADO) {
                Jogador.sendMessage("§c§lFALHA §c> §fN\u00e3o foi poss\u00edvel entrar pois o evento est\u00e1 lotado!");
                return true;
            }
            if (Main.Evento == Estado.INICIANDO) {
                if (PlayerAPI.Participando.size() == 30) {
                    ...
                    if (PlayerAPI.Participando.contains(Jogador)) {
                        Jogador.sendMessage("§cVoc\u00ea j\u00e1 est\u00e1 participando de um evento! Para sair, use: §c§l/sair");
                        return true;
                    }
                    ...
                    if (EventoAPI.Cooldown.containsKey(Jogador)) {
                        Jogador.sendMessage("§7Aguarde §6§l" + EventoAPI.PegarCooldown(Jogador) + " §7segundo(s) para teleportar-se novamente!");
                        return true;
                    }
                    if (Tempo.Iniciando == 0 || Tempo.Iniciando == 1 || Tempo.Iniciando == 2 || Tempo.Iniciando == 3) {
                        Jogador.sendMessage("§cO seu teleporte est\u00e1 sendo cancelado pois o evento est\u00e1 iniciando!");
                        return true;
                    }
                    ...
                    new BukkitRunnable() {
                        public void run() {
                            ...
                            if (Main.Evento == Estado.FECHADO && EventoAPI.Teleportando.contains(Jogador)) {
                                Jogador.sendMessage("§cVoc\u00ea n\u00e3o p\u00f4de se conectar ao evento pois ele foi fechado.");
                                EventoAPI.Teleportando.remove(Jogador);
                                return;
                            }
                            if (EventoAPI.Teleportando.contains(Jogador)) {
                                if (Main.Evento == Estado.FECHADO) {
                                    Jogador.sendMessage("§cN\u00e3o foi poss\u00edvel completar o teleporte pois o evento est\u00e1 fechado!");
                                    return;
                                }
                                Jogador.setExp(0.0f);
                                Jogador.setLevel(0);
                    ...
                    if (PlayerAPI.Participando.contains(Jogador)) {
                        Jogador.sendMessage("§6§lCookie§f§lNetwork §7» §cVoc\u00ea j\u00e1 est\u00e1 participando do evento!");
                        return true;
                    }
                    if (EventoAPI.Teleportando.contains(Jogador)) {
                    ...
                    if (EventoAPI.Cooldown.containsKey(Jogador)) {
                        Jogador.sendMessage("§6§lCookie§f§lNetwork §7» Aguarde §6§l" + EventoAPI.PegarCooldown(Jogador) + " §7segundo(s) para se teleportar novamente.");
                        return true;
                    }
                    if (Tempo.Iniciando == 0 || Tempo.Iniciando == 1 || Tempo.Iniciando == 2 || Tempo.Iniciando == 3) {
                    ...
                    new BukkitRunnable() {
                        public void run() {
                            ...
                            if (Tempo.Iniciando == 0 || Tempo.Iniciando == 1 || Tempo.Iniciando == 2 || Tempo.Iniciando == 3) {
                                Jogador.sendMessage("§cO seu teleporte foi cancelado pois o evento j\u00e1 est\u00e1 iniciando");
                                EventoAPI.Teleportando.remove(Jogador);
                                return;
                            }
                            if (EventoAPI.Teleportando.contains(Jogador)) {
                    ...
                    new BukkitRunnable() {
                        public void run() {
                            ...
                            if (Main.Evento == Estado.FECHADO && EventoAPI.Teleportando.contains(Jogador)) {
                                Jogador.sendMessage("§cO evento foi cancelado. Imposs\u00edvel completar o seu teleporte.");
                                EventoAPI.Teleportando.remove(Jogador);
                                return;
                            }
                            if (EventoAPI.Teleportando.contains(Jogador)) {
                                ...
                                if (PlayerAPI.Participando.size() == 30) {
                                    if (!Jogador.hasPermission("RDM.Full")) {
                                        Jogador.sendMessage("§cN\u00e3o foi poss\u00edvel entrar no evento pois j\u00e1 lotou!");
                                        EventoAPI.Teleportando.remove(Jogador);
                                        return;
                                    }
                                    PlayerAPI.AdicionarRDMCheio(Jogador);
                                ...
                                Jogador.sendMessage("§6§lBOA SORTE §6> §7Tenha um bom jogo. Que a sorte esteja convosco!");
                                ...
                                Tempo.mandarBroadcast("§7O jogador §6§l" + Jogador.getName() + " §7entrou no evento §e§l(" + PlayerAPI.Participando.size() + "§e§l/30)");
            ...
            if (EventoAPI.Cooldown.containsKey(Jogador)) {
                Jogador.sendMessage("§7Aguarde §6§l" + EventoAPI.PegarCooldown(Jogador) + " §7segundo(s) para se teleportar novamente!");
                return true;
            }
            ...
            Tempo.mandarBroadcast("§7O jogador §6§l" + Jogador.getName() + " §7saiu do evento §e§l(" + PlayerAPI.Participando.size() + "§e§l/30)");
