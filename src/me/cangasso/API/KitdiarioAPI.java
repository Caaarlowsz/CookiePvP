...
public class KitdiarioAPI
{
    public static void iniciarContagem(final Player p) {
        Bukkit.getScheduler().scheduleSyncRepeatingTask(Main.getPlugin(), (Runnable)new Runnable() {
            @Override
            public void run() {
                if (cfKitdiario.pegargrupo().get("kitdiario." + p.getUniqueId() + ".dia") != null && (CalendarioAPI.PegarCalendario(CalendarioAPI.Calendario.Dia) > cfKitdiario.pegargrupo().getInt("kitdiario." + p.getUniqueId() + ".dia") + 1 || CalendarioAPI.PegarCalendario(CalendarioAPI.Calendario.Dia) == cfKitdiario.pegargrupo().getInt("kitdiario." + p.getUniqueId() + ".dia") + 1)) {
    ...
    public static void kitdiario(final Player p) {
        if (!cfGrupo.ChecarGrupo(p, "Membro") && !cfGrupo.ChecarGrupo(p, "Light") && !cfGrupo.ChecarGrupo(p, "Mvp") && !cfGrupo.ChecarGrupo(p, "Trial")) {
            p.sendMessage("§cVoc\u00ea j\u00e1 possui todos os kits. N\u00e3o tem o porqu\u00ea de pegar o kit di\u00e1rio!");
            return;
        }
        final Random random = new Random();
        if (cfPermiss\u00e3o.getPermissao(p, "kit.monk") && cfPermiss\u00e3o.getPermissao(p, "kit.ajnin") && cfPermiss\u00e3o.getPermissao(p, "kit.hulk") && cfPermiss\u00e3o.getPermissao(p, "kit.timelord") && cfPermiss\u00e3o.getPermissao(p, "kit.grappler") && cfPermiss\u00e3o.getPermissao(p, "kit.phantom") && cfPermiss\u00e3o.getPermissao(p, "kit.ninja") && cfPermiss\u00e3o.getPermissao(p, "kit.viper") && cfPermiss\u00e3o.getPermissao(p, "kit.snail") && cfPermiss\u00e3o.getPermissao(p, "kit.magma")) {
