...
public class LojaItemMenu implements Listener
{
    ...
    public void abrirInv(final PlayerInteractEvent e) {
        ...
        if ((p.getItemInHand().getType() == Material.EMERALD && API.getWarp(p) == "Spawn" && e.getAction() == Action.RIGHT_CLICK_AIR) || (e.getAction() == Action.RIGHT_CLICK_BLOCK && p.getItemInHand().hasItemMeta() && p.getItemInHand().getItemMeta().hasDisplayName() && p.getItemInHand().getItemMeta().getDisplayName().equals("§6» §7Loja"))) {
            setarInventario(p);
        }
    }
    ...
    public void clicarInv(final InventoryClickEvent e) {
        ...
        if (e.getInventory().getTitle().equals("§7Lojas dos §9§lservidores§7.")) {
            ...
            if (e.getCurrentItem().getType() == Material.EMERALD) {
                ...
                p.closeInventory();
                p.sendMessage(" ");
                p.sendMessage("§6§lCookie§f§lNetwork §7» §fCompre pacotes de §a§lVIP §fou §6§lKITS §facessando: §6§lmc-cookie.com.br");
                p.sendMessage(" ");
            }
        }
    }
